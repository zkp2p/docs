...
**Attestation Service:** The attestation service validates payment evidence and returns an EIP-712 PaymentAttestation. It runs inside an AWS Nitro Enclave for buyer zkTLS (`/verify`), buyer TEE (`/buyer/verify`), and seller (Seller Automated Release) flows, with the EIP-712 signing key KMS-wrapped and PCR8-gated to the enclave. Clients can verify the running enclave via the `/attestation` endpoint before trusting any signed output or sending sensitive session material.
